...
public class ButtonController : MonoBehaviour
{
    ...
    public System.Collections.IEnumerator RedGlitterSequence()
    {
        // kept for compatibility if a timed glitter sequence is needed in future
        _effectsController.ChangeGlitterColor(Color.red);
        _effectsController.ToggleGlitter(true);

        float duration = _objectRotation.getQuickRotationDuration();
    ...
    public void PressButtonRefuse(EffectsController effectsController)
    {
        // Emit a red burst just like green burst for allowed presses
        effectsController.ChangeGlitterBurstColor(Color.red);
        effectsController.GlitterBurst(_objectRotation.getQuickRotationDuration());
    }
