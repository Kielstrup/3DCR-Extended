...
public class Controller : MonoBehaviour
{
    ...
    /// <summary>
    /// Handles the event when an activity execution is refused (disabled activity clicked).
    /// Updates model state and plays an error sound. Visual feedback is handled per-activity.
    /// </summary>
    /// <param name="activityId">The ID of the activity whose execution was refused.</param>
    private void OnActivityExecuteRefused(string activityId)
    {   
        // on refuse execute entry update the State
        _model.CheckAndRecordActivityStates();
        // Visual feedback is handled per-activity (ViewActivity/ButtonController).
        // Avoid triggering a global glitter burst here to prevent duplicate bursts on multiple buttons.
        if(GameSettings.ActiveCamera != CameraMode.BirdsEye)
        {   
